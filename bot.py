...
def save_memory():
    try:
        data = {k: {"last_used": v.get("last_used", time.time())} for k, v in conversation_memory.items()}
...
def cleanup_memory():
    ...
    for cid in list(conversation_memory.keys()):
        if now - conversation_memory[cid].get("last_used", 0) > MEMORY_TIMEOUT:
...
@app_commands.describe(prompt="Image description", aspect_ratio="Ratio", resolution="Quality", num_images="Count (1-4)")
@app_commands.choices(
    aspect_ratio=[
        ...
        app_commands.Choice(name="2:3 Portrait/Tool", value="2:3"),
    ],
    resolution=[
        app_commands.Choice(name="1K Standard", value="1k"),
        app_commands.Choice(name="2K High", value="2k"),
    ]
)
async def imagine(interaction: discord.Interaction, prompt: str, aspect_ratio: str = "1:1", resolution: str = "1k", num_images: int = 1):
    ...
    try:
        ...
        await interaction.followup.send(f"❌ Image generation failed: {str(e)[:300]}")


@tree.command(name="stats", description="Bot statistics")
async def stats(interaction: discord.Interaction):
    embed = discord.Embed(title="Grok Bot Status", color=0x1DA1F2)
...
async def help_command(interaction: discord.Interaction):
    embed = discord.Embed(title="Grok Bot Commands", description="Powered by xAI", color=0x1DA1F2)
...
async def reload(interaction: discord.Interaction):
    if interaction.user.id != BOT_OWNER_ID:
        ...
        return
    await interaction.response.send_message("🔄 Saving & restarting...")
    save_memory()
